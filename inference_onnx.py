...
def generate_top_down_view(frame, P, pitch_width=1050, pitch_height=680):
    # Generates a 2D Homography Tactical Map
    H = np.zeros((3, 3))
    H[:, 0] = P[:, 0]
    H[:, 1] = P[:, 1]
    H[:, 2] = P[:, 3]
...
def process_batch(cam, frames_buffer, raw_frames_buffer, session_kp, session_line, kp_threshold, line_threshold, pnl_refine, device, out):
    ...
    for i in range(len(raw_frames_buffer)):
        ...
        if final_params_dict is not None:
            ...
            # Draw 3D lines on the raw frame
            projected_frame = project(raw_frame.copy(), P)
            
            # Generate the 2D Tactical Map
            top_down_view = generate_top_down_view(raw_frame, P)
            
            # Resize the map to attach cleanly next to the original frame
            ...
            # If tracking fails, just show the original frame
...
def process_video(input_path, save_path, session_kp, session_line, kp_threshold, line_threshold, pnl_refine, device, batch_size):
    ...
    # We double the width of the output video to fit both the broadcast and the tactical map side-by-side
